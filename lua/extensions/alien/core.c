...
static void pusherror_ (lua_State *L) {
...
static const ffi_abi ffi_abis[] = {
#if defined(_MSC_VER)
  #if _WIN64
    FFI_DEFAULT_ABI, FFI_WIN64
  #else
    FFI_DEFAULT_ABI, FFI_MS_CDECL, FFI_STDCALL
  #endif
#elif defined(DARWIN)
    #define FFI_SYSV FFI_DEFAULT_ABI
    FFI_DEFAULT_ABI, FFI_SYSV, FFI_STDCALL
#else
...
static const char *const ffi_abi_names[] = {
    "default"
#if defined(_MSC_VER)
  #if _WIN64
    "win64",
...
static void alien_callback_call(ffi_cif *cif, void *resp, void **args, void *data) {
  ...
  case AT_STRING:
    if(lua_isuserdata(ac->L, -1))
      *((char**)resp) = (char*)lua_touserdata(ac->L, -1);
    else
...
static int alien_callback_new(lua_State *L) {
  ...
  if(ac != NULL && ud != NULL) {
    int j;
    *ud = (ffi_closure*)malloc_closure();
    if(*ud == NULL) { free(ac); luaL_error(L, "alien: cannot allocate callback"); }
    ac->L = L;
    if(lua_istable(L, 2)) {
      lua_getfield(L, 2, "ret");
      ac->ret_type = (alien_Type)types[luaL_checkoption(L, -1, "int", typenames)];
      ...
      ac->ret_type = (alien_Type)types[luaL_checkoption(L, 2, "int", typenames)];
  ...
  ac->ffi_params[i] = ffitypes[luaL_checkoption(L, -1, "int", typenames)];
    ac->params[i] = (alien_Type)types[luaL_checkoption(L, -1, "int", typenames)];
  lua_pop(L, 1);
      }
    } else {
      for(i = 0, j = 3; i < ac->nparams; i++, j++) {
  ac->ffi_params[i] = ffitypes[luaL_checkoption(L, j, "int", typenames)];
    ac->params[i] = (alien_Type)types[luaL_checkoption(L, j, "int", typenames)];
      }
    }
    lua_pushvalue(L, 1);
...
static int alien_function_types(lua_State *L) {
  ...
  if(lua_istable(L, 2)) {
    ...
    af->ret_type = (alien_Type)types[ret_type];
  ...
  if(lua_istable(L, 2)) {
    for(i = 0, j = 1; i < af->nparams; i++, j++) {
      ...
      af->params[i] = (alien_Type)types[luaL_checkoption(L, -1, "int", typenames)];
      lua_pop(L, 1);
    }
  } else {
    for(i = 0, j = 3; i < af->nparams; i++, j++) {
      ...
      af->params[i] = (alien_Type)types[type];
    }
  }
...
static int alien_function_call(lua_State *L) {
  ...
  int *refi_args = NULL, nrefi, nrefd, nrefc;
  ...
  if(nargs > 0) args = (void**)ALLOCA(sizeof(void*) * nargs);
  for(i = 0, j = 2; i < nparams; i++, j++) {
    ...
    *((char**)arg) = lua_isnil(L, j) ? NULL : (char*)lua_touserdata(L, j);
      else
...
#define pvoidf void(*)()
  switch(af->ret_type) {
  case AT_VOID: ffi_call(cif, (pvoidf)af->fn, NULL, args); lua_pushnil(L); break;
  case AT_SHORT: ffi_call(cif, (pvoidf)af->fn, &iret, args); lua_pushnumber(L, (short)iret); break;
  case AT_LONG: 
    ffi_call(cif, (pvoidf)af->fn, &lret, args); lua_pushnumber(L, lret); break;
  case AT_INT: ffi_call(cif, (pvoidf)af->fn, &iret, args); lua_pushnumber(L, iret); break;
  case AT_CHAR: ffi_call(cif, (pvoidf)af->fn, &iret, args); lua_pushnumber(L, (uchar)iret); break;
  case AT_BYTE: ffi_call(cif, (pvoidf)af->fn, &iret, args); lua_pushnumber(L, (signed char)iret); break;
  case AT_FLOAT: ffi_call(cif, (pvoidf)af->fn, &fret, args); lua_pushnumber(L, fret); break;
  case AT_DOUBLE: ffi_call(cif, (pvoidf)af->fn, &dret, args); lua_pushnumber(L, dret); break;
  case AT_STRING: ffi_call(cif, (pvoidf)af->fn, &pret, args);
    (pret ? lua_pushstring(L, (const char *)pret) : lua_pushnil(L)); break;
  case AT_PTR: ffi_call(cif, (pvoidf)af->fn, &pret, args);
...
static int alien_buffer_get(lua_State *L) {
  static const void* funcs[] = {
      ...
      (void*)&alien_buffer_len,
      (void*)&alien_buffer_get,
      (void*)&alien_buffer_put };
  
  static const char *const funcnames[] = {
      "tostring", "topointer", "len", "get", "set", NULL };
  
  static const int types[] = {
      AT_VOID, AT_INT, AT_DOUBLE, AT_CHAR, AT_STRING, AT_PTR, AT_REFINT, 
      AT_REFDOUBLE, AT_REFCHAR, AT_CALLBACK, AT_SHORT, AT_BYTE, AT_LONG,
      AT_FLOAT };
  ...
  else
  {
    ...
    case AT_CALLBACK: 
      p = *((void**)&b[offset]);
      if (p)
          alien_makefunction(L, NULL, p, NULL);
      else
          lua_pushnil(L);
      break; 
    case AT_PTR:
...
static int alien_buffer_put(lua_State *L) {
  ...
  case AT_STRING: *((char**)(&b[offset])) = (lua_isnil(L, 3) ? NULL : (char*)lua_tostring(L, 3)); break;
  case AT_CALLBACK: *((void**)(&b[offset])) = alien_checkcallback(L, 3); break;
  case AT_PTR: *((void**)(&b[offset])) = (lua_isnil(L, 3) ?
      NULL : (lua_isuserdata(L, 3) ? lua_touserdata(L, 3) : (void*)lua_tostring(L, 3))); break;
  default: 
