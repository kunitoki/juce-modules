...
static int Lrealpath(lua_State *L) {
    ...
    if (pfn != NULL)
    {
        ...
        bytes = (*pfn)(hFile, p, MAX_PATH, 0);
        if (bytes > MAX_PATH) {
            p = (LPWSTR)lua_newuserdata(L, bytes * sizeof(WCHAR));
            bytes = (*pfn)(hFile, p, bytes, 0);
        }
        CloseHandle(hFile);
...
static const char *classend2(const char *p) {
    const char *op = p;
    if (p[1] == '^') ++p;
    while (*++p != '\0' && *p != ']')
        ;
...
static int __fnmatch(const char *pattern, const char *s, size_t len) {
    ...
    while (*pattern != '\0') {
        ...
        case '*': case '?':
            ...
            if (s_end - s < min)
                return 0;
            if (!hasmax)
                return __fnmatch(pattern, s+min, (s_end-s) - min);
            len = (s_end-s) - min;
            for (i = 0; i <= len; ++i) {
                if (__fnmatch(pattern, s_end-i, i))
                    return 1;
            }
            return 0;
        case '[':
            ec = classend2(pattern);
            if (ec != pattern) {
...
static int glob_walker(WalkState *S) {
    ...
    if (len != 0 && __fnmatch(gs->pattern, s, len)) {
        lua_pushvalue(S->L, -1);
...
static int L__fnmatch(lua_State *L) {
    size_t len;
    const char *s = luaL_checklstring(L, 1, &len);
    const char *pattern = luaL_checkstring(L, 2);
    lua_pushboolean(L, __fnmatch(pattern, s, len));
    return 1;
}
