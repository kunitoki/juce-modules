...
static const char *memdisk_error[] = {
    "Ok",
...
zzip_mem_disk_open(char *filename)
{
    ...
    if (! disk)
        { perror(memdisk_error[_zzip_mem_disk_open_fail]); return 0; }
...
zzip_mem_disk_fdopen(int fd)
{
    ...
    if (! disk)
        { perror(memdisk_error[_zzip_mem_disk_fdopen_fail]); return 0; }
    ___ ZZIP_MEM_DISK *dir = zzip_mem_disk_new();
    zzip_mem_disk_load(dir, disk);
    return dir;
    ____;
...
zzip_mem_disk_buffer(char *buffer, size_t buflen)
{
    ...
    if (! disk)
        { perror(memdisk_error[_zzip_mem_disk_buffer_fail]); return 0; }
    ___ ZZIP_MEM_DISK *dir = zzip_mem_disk_new();
    zzip_mem_disk_load(dir, disk);
    return dir;
    ____;
