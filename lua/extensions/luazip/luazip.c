...
static int luazip_pushresult (lua_State *L, int i, const char *filename) {
  if (i) {
    lua_pushboolean(L, 1);
    return 1;
  }
...
static ZZIP_DIR** luazip_topfile (lua_State *L, int findex) {
  ZZIP_DIR** f = (ZZIP_DIR**)luaL_checkudata(L, findex, ZIPFILEHANDLE);
  if (f == NULL) luaL_argerror(L, findex, "bad zip file");
  return f;
}

static ZZIP_DIR* luazip_tofile (lua_State *L, int findex) {
  ZZIP_DIR** f = luazip_topfile(L, findex);
  if (*f == NULL)
    luaL_error(L, "attempt to use a closed zip file");
  return *f;
}

static ZZIP_FILE** luazip_topinternalfile (lua_State *L, int findex) {
  ZZIP_FILE** f = (ZZIP_FILE**)luaL_checkudata(L, findex, ZIPINTERNALFILEHANDLE);
  if (f == NULL) luaL_argerror(L, findex, "bad zip file");
  return f;
}

static ZZIP_FILE* luazip_tointernalfile (lua_State *L, int findex) {
  ZZIP_FILE** f = luazip_topinternalfile(L, findex);
  if (*f == NULL)
    luaL_error(L, "attempt to use a closed zip file");
  return *f;
}
...
*/
static ZZIP_DIR** luazip_newfile (lua_State *L) {
...
static ZZIP_FILE** luazip_newinternalfile (lua_State *L) {
...
static int luazip_zip_open (lua_State *L) {
  const char *zipfilename = luaL_checkstring(L, 1);
  /*const char *mode = luaL_optstring(L, 2, "r");*/

  ZZIP_DIR** pf = luazip_newfile(L);
...
static int luazip_zip_close (lua_State *L) {
  ZZIP_DIR* f = luazip_tofile(L, 1);
...
static int luazip_f_open (lua_State *L) {
  ZZIP_DIR* uf = luazip_tofile(L, 1);
  const char *filename = luaL_checkstring(L, 2);
  /*const char *mode = luaL_optstring(L, 3, "r");*/
  ZZIP_FILE** inf = luazip_newinternalfile(L);
...
*/
static int luazip_zip_openfile (lua_State *L) {
  ZZIP_FILE** inf;
...
static int luazip_zip_type (lua_State *L) {
...
static int luazip_zip_tostring (lua_State *L) {
  char buff[32];
  ZZIP_DIR** f = luazip_topfile(L, 1);
  if (*f == NULL)
    strcpy(buff, "closed");
  else
...
static int luazip_ff_tostring (lua_State *L) {
  char buff[32];
  ZZIP_FILE** f = luazip_topinternalfile(L, 1);
  if (*f == NULL)
    strcpy(buff, "closed");
  else
    sprintf(buff, "%p", lua_touserdata(L, 1));
...
static int luazip_zip_gc (lua_State *L) {
  ZZIP_DIR**f = luazip_topfile(L, 1);
...
static int luazip_zip_readfile (lua_State *L) {
...
static int luazip_f_files (lua_State *L) {
  ZZIP_DIR *f = luazip_tofile(L, 1);
  zzip_rewinddir(f);
  lua_pushliteral(L, ZIPFILEHANDLE);
  lua_rawget(L, LUA_REGISTRYINDEX);
  lua_pushcclosure(L, luazip_zip_readfile, 2);
  return 1;
}

static int luazip_aux_close (lua_State *L) {
  ZZIP_FILE *f = luazip_tointernalfile(L, 1);
...
static int luazip_ff_close (lua_State *L) {
  return luazip_pushresult(L, luazip_aux_close(L), NULL);
}

static int luazip_ff_gc (lua_State *L) {
  ZZIP_FILE**f = luazip_topinternalfile(L, 1);
  if (*f != NULL)  /* ignore closed files */
    luazip_aux_close(L);
  return 0;
...
static int luazip_zzip_getc (ZZIP_FILE *f)
{
  char c;
  return (zzip_fread(&c, sizeof(char), 1, f) == 0) ? EOF : (int)c;
}

static char* luazip_zzip_fgets(char *str, int size, ZZIP_FILE *stream)
{
  int c, i;

    for (i = 0; i < size-1; i++)
    {
    c = luazip_zzip_getc(stream);
        if (EOF == c)
            return NULL;
        str[i]=c;
        if (('\n' == c)/* || ('\r' == c)*/)
...
static int luazip_test_eof (lua_State *L, ZZIP_FILE *f) {
  /* TODO */
    (void) L;
    (void) f;
  return 1;
...
static int luazip_read_line (lua_State *L, ZZIP_FILE *f) {
  luaL_Buffer b;
  luaL_buffinit(L, &b);
  for (;;) {
    size_t l;
    char *p = luaL_prepbuffer(&b);
    if (luazip_zzip_fgets(p, LUAL_BUFFERSIZE, f) == NULL) {  /* eof? */
...
static int luazip_read_chars (lua_State *L, ZZIP_FILE *f, size_t n) {
...
static int luazip_g_read (lua_State *L, ZZIP_FILE *f, int first) {
  int nargs = lua_gettop(L) - 1;
  int success;
  int n;
  if (nargs == 0) {  /* no arguments? */
    success = luazip_read_line(L, f);
    ...
    for (n = first; nargs-- && success; n++) {
      if (lua_type(L, n) == LUA_TNUMBER) {
        ...
        success = (l == 0) ? luazip_test_eof(L, f) : luazip_read_chars(L, f, l);
      }
      else {
        ...
        switch (p[1]) {
          case 'l':  /* line */
            ...
            break;
          case 'a':  /* file */
            luazip_read_chars(L, f, ~((size_t)0));  /* read MAX_SIZE_T chars */
...
static int luazip_ff_read (lua_State *L) {
  return luazip_g_read(L, luazip_tointernalfile(L, 1), 2);
}

static int luazip_zip_readline (lua_State *L);

static void luazip_aux_lines (lua_State *L, int idx, int close) {
  lua_pushliteral(L, ZIPINTERNALFILEHANDLE);
  lua_rawget(L, LUA_REGISTRYINDEX);
  lua_pushvalue(L, idx);
  lua_pushboolean(L, close);  /* close/not close file when finished */
  lua_pushcclosure(L, luazip_zip_readline, 3);
}

static int luazip_ff_lines (lua_State *L) {
  luazip_tointernalfile(L, 1);  /* check that it's a valid file handle */
  luazip_aux_lines(L, 1, 0);
  return 1;
}
...
static int luazip_zip_readline (lua_State *L) {
  ...
  if (f == NULL)  /* file is already closed? */
    luaL_error(L, "file is already closed");
  if (luazip_read_line(L, f)) return 1;
  else {  /* EOF */
    if (lua_toboolean(L, lua_upvalueindex(3))) {  /* generator created file? */
      lua_settop(L, 0);
      lua_pushvalue(L, lua_upvalueindex(2));
      luazip_aux_close(L);  /* close it */
    }
    return 0;
  }
}

static int luazip_ff_seek (lua_State *L) {
  ...
  if (op < 0)
    return luazip_pushresult(L, 0, NULL);  /* error */
  else {
...
static const luaL_reg luazip_ziplib[] = {
  {"open", luazip_zip_open},
  {"close", luazip_zip_close},
  {"type", luazip_zip_type},
//  {"files", io_files},
  {"openfile", luazip_zip_openfile},
  {NULL, NULL}
};

static const luaL_reg luazip_flib[] = {
  {"open", luazip_f_open},
  {"close", luazip_zip_close},
  {"files", luazip_f_files},
  {"__gc", luazip_zip_gc},
  {"__tostring", luazip_zip_tostring},
  {NULL, NULL}
};

static const luaL_reg luazip_fflib[] = {
  {"read", luazip_ff_read},
  {"close", luazip_ff_close},
  {"seek", luazip_ff_seek},
  {"lines", luazip_ff_lines},
  {"__gc", luazip_ff_gc},
  {"__tostring", luazip_ff_tostring},
/*  {"flush", ff_flush},
  {"write", ff_write},*/
  {NULL, NULL}
};
...
*/
static void luazip_set_info (lua_State *L) {
...
static void luazip_createmeta (lua_State *L) {
  ...
  lua_pushvalue(L, -2);  /* push metatable */
  lua_rawset(L, -3);  /* metatable.__index = metatable */
  luaL_openlib(L, NULL, luazip_fflib, 0);
}

LUALIB_API int luaopen_zip (lua_State *L) {
  luazip_createmeta(L);
  lua_pushvalue(L, -1);
  luaL_openlib(L, LUA_ZIPLIBNAME, luazip_ziplib, 1);
  luazip_set_info(L);
  return 1;
}
...
#include "zzip/dir.c"
#include "zzip/err.c"
#include "zzip/fetch.c"
#include "zzip/file.c"
#include "zzip/fseeko.c"
#include "zzip/info.c"
#include "zzip/memdisk.c"
#include "zzip/mmapped.c"
#include "zzip/plugin.c"
#include "zzip/stat.c"
#include "zzip/write.c"
#include "zzip/zip.c"
